...
public:
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
		...
		handler->PSendSysMessage("Ядро: UAG Core");
        handler->PSendSysMessage("Сайт: wow-atlantida.ru");
		handler->PSendSysMessage("Developers: [Dev]Jobtq | [Dev]Roman | [Dev] @darkteam");
		handler->PSendSysMessage("Last update: 29.12.2014");
