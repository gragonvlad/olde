...
public:
	...
	bool CreateArenateam(Player* player, Creature* me)
	{
		...
		ChatHandler(player->GetSession()).SendSysMessage("1v1 Арена тим создана!");
	...
	bool OnGossipHello(Player* player, Creature* me)
	{
		if(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_1v1)) == NULL)
			player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Создать 1v1 рейтинговую тиму", GOSSIP_SENDER_MAIN, 1, "Хотите создать арену 1v1?", ARENA_1V1_COST, false);
		else
		{
			if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_1v1))
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Покинуть 1v1 Арена тиму", GOSSIP_SENDER_MAIN, 3);
			else
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_3v3_5:30|t Присоедениться к арене 1v1", GOSSIP_SENDER_MAIN, 2);
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30|t Выйти из тимы 1v1", GOSSIP_SENDER_MAIN, 5, "Вы уверены?", 0, false);
			}

			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/INV_Misc_Coin_01:30|t Посмотреть статистику", GOSSIP_SENDER_MAIN, 4);
		}

		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface/ICONS/INV_Misc_Coin_03:30|t Как работает этот НПЦ?", GOSSIP_SENDER_MAIN, 8);
	...
	bool OnGossipSelect(Player* player, Creature* me, uint32 /*uiSender*/, uint32 uiAction)
	{
		...
		case 5: // Disband arenateam
			{
				...
				ChatHandler(player->GetSession()).PSendSysMessage("Команда арены 1v1 удалена!");
				player->CLOSE_GOSSIP_MENU();
				return true;
			}
			break;
		...
		case 8: // Script Info
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Разработчик: Jobtq", GOSSIP_SENDER_MAIN, uiAction);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Сайт: WoW-Atlantida.ru", GOSSIP_SENDER_MAIN, uiAction);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Гладиатор сезона: Сезон продолжается!", GOSSIP_SENDER_MAIN, uiAction);
